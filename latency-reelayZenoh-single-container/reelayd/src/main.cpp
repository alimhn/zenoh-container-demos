...
struct Context {
    reelay::monitor<simdjson::dom::element, reelay::json>* monitor;
    z_owned_publisher_t publisher;
};

void monitor_on_sample(const z_sample_t* sample, void* context)
{
  //cast the context to the Context struct
  ...
  // }
  z_publisher_t pub = z_loan(ctx->publisher);
  #ifdef ZENOH_C
...
int main(int argc, char** argv)
{
  ...
  Context ctx = {&monitor, pub};

  const char* keyexpr = vm["key"].as<std::string>().c_str();

  z_owned_closure_sample_t callback = z_closure(monitor_on_sample, nullptr, &ctx);
  z_owned_subscriber_t sub = z_declare_subscriber(z_loan(session), z_keyexpr(keyexpr), z_move(callback), nullptr);
